...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  response.send(result(responseResult));
});
//API-3
...
app.put("todos/:todoId/", async (request, response) => {
  ...
  const previousTodoQuery = `SELECT*FROMtodoWHERE id = '${todoId}';`;
  ...
  switch (true) {
    case requestBody.status != undefined:
      if (status == "TO DO" || status == "IN PROGRESS" || status == "DONE") {
        updateTodoQuery = `UPDATE todo SET todo='${todo}',priority='${priority}',status='${status}',category='${category}',dueDate='${dueDate}' WHERE todoId='${todoId}';`;
    ...
    case requestBody.category != undefined:
      if (category == "WORK" || category == "HOME" || category == "LEARNING") {
        updateTodoQuery = `UPDATE todo SET todo='${todo}',priority='${priority}',status='${status}',category='${category}',dueDate='${dueDate}';`;
        await db.run(updateTodoQuery);
    ...
    case requestBody.dueDate != undefined:
      if (isMatch(dueDate, "yyyy-MM-dd")) {
        ...
        updateTodoQuery = `UPDATE todo SET todo='${todo}',priority='${priority}',status='${status}',category='${category}',dueDate='${newDueDate}' WHERE id='${todoId}';`;
        await db.run(updateTodoQuery);
